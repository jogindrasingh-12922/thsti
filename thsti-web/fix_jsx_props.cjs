...
updateFile(path.join(srcDir, 'components/layout/Accessibility.jsx'), (content) => {
    ...
    // Also remove the extra copy from line 217-260 (using regex block removing)
    const extraPattern = /\{\/\*  Accessibility Tools  \*\/\}.*?<\/div>(\s*)<\/div>/s;
    const match = newContent.match(extraPattern);
    if (match) {
