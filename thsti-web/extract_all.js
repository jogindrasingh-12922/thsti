...
function extractSection(startStr, endStr, compName, isHeaderOrFooter = false) {
    ...
    snippet = snippet.replace(/<source(.*?)>/gis, (match, p1) => {
        if (p1.endsWith('/')) return match;
        return `<source${p1} />`;
    });
    ...
    // Replace HTML comments (multiline)
    ...
    // fixes for stray tags
    snippet = snippet.replace(/alt="" \/>/g, 'alt="" />');

    // React specific fixes for Header (e.g. href="javascript:void(0);")
    snippet = snippet.replace(/href="javascript:void\(0\);"/g, 'href="#"');

    const baseDir = isHeaderOrFooter ? 'src/components/layout' : 'src/components/home';
    if (!fs.existsSync(baseDir)) fs.mkdirSync(baseDir, { recursive: true });

    const outputFn = `${baseDir}/${compName}.jsx`;
...
// Layout
extractSection('<header class="main-header header-style-two">', '</header>', 'Header', true);
extractSection('<footer class="main-footer">', '</footer>', 'Footer', true);
extractSection('<div id="sidebar"', '<!--Scroll to top-->', 'Accessibility', true);
...
marqueeSnippet = marqueeSnippet.replace(/class="/g, 'className="').replace(/<!--(.*?)-->/gs, '{/* $1 */}').replace(/<img(.*?)>/gis, (m, p) => { if (p.endsWith('/')) return m; return `<img${p} />`; });
...
extractSection('<section class="what-we-offer" style="background-image:url(images/background/5-2.jpg);">', '</section>', 'TabsSection');
